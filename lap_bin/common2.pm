...
sub process_lsf_log_line(@)
{
		...
		if ($line =~ /Subject:\s+Job\s+(\S+):/)
		{
				...
				$log_output_ref->{$cur_job} = $$cur_output_ref if ($$cur_job_ref && $$cur_output_ref && $jobs_ref{$$cur_job_ref});
				$$cur_job_ref = $job;
		}
...
sub process_sge_log_line(@)
{
		...
		if ($line =~ /^JobId:\s+(\S+):/)
		{
				...
				$log_output_ref->{$cur_job} = $$cur_output_ref if ($$cur_job_ref && $jobs_ref->{$$cur_job_ref});
				$$cur_job_ref = $job;
				$$cur_output_ref = "";

				my $exit_status = undef;
				my $qacct_output = `qacct -j $cur_job`;
				my @lines = split /\n/, $qacct_output;
				foreach (@lines)
				{
						chomp;
						my @cols = split(/\s+/, $_);
						if ($cols[0] eq "exit_status")
						{
								$exit_status = $cols[1];
								last;
						}
