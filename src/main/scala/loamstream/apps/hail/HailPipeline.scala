...
import loamstream.model.values.LType.{LGenotype, LInt, LSampleId, LVariantId}
...
case class HailPipeline(genotypesId: String, vdsId: String, singletonsId: String) extends LPipeline {
  ...
  val singletonPile = LPile(singletonsId, LSig.Map(LTuple1(LSampleId), LInt), PileKinds.singletonCounts)
