...
import loamstream.compiler.CompilationError
import LoamCompiler.CompilerReporter
import loamstream.compiler.LoamClassLoader
import loamstream.compiler.LoamProject
import loamstream.compiler.ReportableCompilationError
...
object LoamCompiler extends Loggable {
  ...
  object Result {
    ...
    def success(warnings: Seq[Issue], infos: Seq[Issue], graph: LoamGraph): Success = {
      Success(warnings, infos, graph)
    }
...
final class LoamCompiler(
  ...
  def evaluateLoamFilesV2(loamConfig: LoamConfig, pkg: String): Unit = {
    val scanner: Reflections = new Reflections(
      (new ConfigurationBuilder)
        .setUrls(ClasspathHelper.forPackage(pkg))
        .setScanners(new SubTypesScanner))
  
    def loamFileClassNames: Set[String] = {
      import JavaConverters._
      
      scanner.getSubTypesOf(classOf[LoamFile]).asScala.map(_.getName).toSet
    }
  
    LoamFile.config = loamConfig
    
    LoamFile.clearGraph()
  
    println(s"@@@@@@@@@ Starting Loam evaluation, graph has ${LoamFile.graph.tools.size} tools.")
    
    //val classLoader = new LoamClassLoader(getClass.getClassLoader)
    //val classLoader = getClass.getClassLoader
    val classLoader = new AbstractFileClassLoader(targetDirectory, getClass.getClassLoader)
  
    val classNames = loamFileClassNames
    ...
    println(s"@@@@@@@@@ Done with Loam evaluation, graph has ${LoamFile.graph.tools.size} tools.")
  }
  
  def compileV2(loamConfig: LoamConfig, files: Path*): LoamCompiler.Result = compileV2(loamConfig, "", files: _*)
  
  def compileV2(loamConfig: LoamConfig, pkg: String, files: Path*): LoamCompiler.Result = compileLock.synchronized {
    try {
      val sourceFiles = files.map(f => new BatchSourceFile(f.toString, Files.readFrom(f)))
      ...
      if (targetDirectory.nonEmpty) {
        ...
        val scriptBoxes = TimeUtils.time("Evaluating Loam code", debug(_)) {
          evaluateLoamFilesV2(loamConfig, pkg)
        }

        val graph = LoamFile.graph
        ...
        LoamCompiler.Result.success(reporter.warnings, reporter.infos, graph)
      } else {
