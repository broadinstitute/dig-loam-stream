...
object LanguageSupport {
  object R {
    implicit final class StringContextWithR(val stringContext: StringContext) extends AnyVal {
      ...
      def r(
           args: Any*)
          (name: String = LoamCmdTool.autoGeneratedName())(implicit scriptContext: LoamScriptContext): LoamCmdTool = {
        
        val rConfig = scriptContext.rConfig
        
        val scriptFile = makeScript(stringContext, name, GeneratedScriptParams.r(rConfig), args: _*)
        
        import LoamCmdTool._

        cmd"${rConfig.binary} $scriptFile"(name)
      }
    }
  }
  ...
  object Python {
    implicit final class StringContextWithPython(val stringContext: StringContext) extends AnyVal {
      ...
      def python(
        ...
        val scriptFile = makeScript(stringContext, name, GeneratedScriptParams.python(pythonConfig), args: _*)
        
        import LoamCmdTool._

        cmd"${pythonConfig.binary} $scriptFile"(name)
      }
    }
  }
  ...
  def makeScriptContent(
    ...
    LoamCmdTool.create(args: _*)(name, StringUtils.assimilateLineBreaks)(scriptContext, stringContext).commandLine
  }
  ...
  def makeScript(
    ...
    val scriptContent = makeScriptContent(stringContext, name, args: _*)
