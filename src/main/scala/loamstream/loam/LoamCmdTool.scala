...
object LoamCmdTool {
  ...
  val autogeneratedNamePrefix: String = "$cmd"
  
  private def autoGeneratedName(): String = s"${autogeneratedNamePrefix}-${nameSequence.next()}"
  ...
  private def addToGraph(tool: LoamCmdTool)(implicit scriptContext: LoamScriptContext): Unit = {
    scriptContext.projectContext.updateGraph { graph =>
      graph.withTool(tool, scriptContext).withToolName(tool, autoGeneratedName())
    }
  }
