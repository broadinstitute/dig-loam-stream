...
object LoamGraph {
  ...
  lazy val empty: LoamGraph = {
    LoamGraph(
      _stores = new java.util.HashSet,
      _tools = new java.util.HashSet,
  ...
  def autoGeneratedToolName(): String = s"${autogeneratedToolNamePrefix}-${toolNameSequence.next()}"
...
final case class LoamGraph(
    private val _stores: java.util.Set[Store],
    private val _tools: java.util.Set[Tool],
  ...
  def stores: Iterable[Store] = _stores.iterator.asScala.toIterable
  
  def tools: Iterable[Tool] = _tools.iterator.asScala.toIterable
  
  /** Returns graph with store added */
  def withStore(store: Store): LoamGraph = { _stores.add(store) ; this }//copy(stores = stores + store)
  ...
  def initialTools: Iterable[Tool] = _tools.iterator.asScala.filter(toolsPreceding(_).isEmpty).toIterable

  /** All tools with no succeeding tools */
  def finalTools: Iterable[Tool] = _tools.iterator.asScala.filter(toolsSucceeding(_).isEmpty).toIterable
  ...
  def ranks: Map[Tool, Int] = {
    val initialRanks: Map[Tool, Int] = _tools.iterator.asScala.map(tool => (tool, 0)).toMap

    _tools.iterator.asScala.foldLeft(initialRanks) { (ranks, tool) =>
  ...
  def without(toolsToExclude: Set[Tool]): LoamGraph = containingOnly(_tools.asScala.toSet -- toolsToExclude)
  
  private def containingOnly(toolsToKeep: Set[Tool]): LoamGraph = {

    if(toolsToKeep == _tools.asScala) { this }
    else {
      ...
      LoamGraph(
          _stores = retainedStores.asJava,
          _tools = toolsToKeep.asJava,
