    ...
    private[dga] final class UploadOps(
      ...
      private def processDownload(download: Annotation.Download): Unit = {
		...
		val (handle, bedRowAttempts) = annotation.annotationType match {
          case AnnotationType.TargetGenePredictions => Beds.downloadBed(url, auth, Some(Seq("chr","start","end","name","class","TargetGene","TargetGeneEnsemblID","TargetGeneTSS","CellType","Score","DistanceToTSS","TargetGeneElementStart","TargetGeneElementEnd")))
