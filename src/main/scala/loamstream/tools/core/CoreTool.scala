...
object CoreTool {
  ...
  final case class ExtractSampleIdsFromVcfFile(
    ...
    override val id = LNamedId(s"Extracting sample ids from '$vcfFile' and storing them in '$sampleFile'")
  }
  ...
  final case class ConvertVcfToVds(
    ...
    override val id = LNamedId(s"Convert '$vcfFile' into Hail format VDS dir '$vdsDir'.")
  }
  ...
  final case class CalculateSingletons(
    ...
    override val id = LNamedId(s"Calculate singletons from genotype calls in VDS format at '$vdsDir'.")
  }
  ...
  final case class ProjectPcaNative(vcfFile: Path, pcaWeightsFile: Path, klustaConfig: KlustaKwikKonfig) extends 
    ...
    override val id = LNamedId(s"Project PCA using native method: vcf: '$vcfFile', pca weights: '$pcaWeightsFile'.")
  }
  
  final case class ProjectPca(vcfFile: Path, pcaWeightsFile: Path, klustaConfig: KlustaKwikKonfig) extends 
      Tool.Nary((vcfStore(vcfFile), pcaWeightsStore(pcaWeightsFile)) ~> pcaProjectedStore(klustaConfig.inputFile)) {
    
    override val id = LNamedId(s"Project PCA: vcf: '$vcfFile', pca weights: '$pcaWeightsFile'.")
  }
  ...
  final case class KlustaKwikClustering(klustaConfig: KlustaKwikKonfig) extends 
    ...
    override val id = LNamedId(s"KlustaKwik Clustering: pca projected file '${klustaConfig.inputFile}', sample cluster file: '${klustaConfig.outputFile}'.")
  }
  
  final case class ClusteringSamplesByFeatures(klustaConfig: KlustaKwikKonfig) extends 
      Tool.OneToOne(pcaProjectedStore(klustaConfig.inputFile) ~> sampleClusterStore(klustaConfig.outputFile)) {
    
    override val id = LNamedId(s"Clustering Samples by Features: pca projected file '${klustaConfig.inputFile}', sample cluster file: '${klustaConfig.outputFile}'.")
  }
}
