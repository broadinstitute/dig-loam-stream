...
final class OutputAndExecutionRecordingTest extends FunSuite with ProvidesSlickLoamDao {
  test("Data about outputs and job executions should be recorded properly") {
    ...
    val graph = TestHelpers.makeGraph { implicit context =>
      ...
      local {
        cmd"(echo copying locally 0 ; cp $in $out0)"(name = "Local-copy0", in = Seq(in), out = Seq(out0))

        cmd"(echo copying locally 1 ; cp $out0 $out1)"(name = "Local-copy1", in = Seq(out0), out = Seq(out1))
        
        cmd"(echo copying locally 2 ; cp $out1 $out2)"(name = "Local-copy2", in = Seq(out1), out = Seq(out2))
      }
    }
