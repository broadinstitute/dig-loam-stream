...
final class AndThenTest extends FunSuite with Loggable {
  ...
  test("split slightly-less-simple loam file") {
    val code = """
      ...
      cmd"foo -i $store0 -o $store1"(in = Seq(store0), out = Seq(store1))
      ...
      andThen {
        ...
        for(i <- 1 to N) {
          ...
          cmd"bar -i $store1 -o $barOutput"(in = Seq(store1), out = Seq(barOutput))
        }
      }
      """
  ...
  test("split loam file with commands before and after andThen") {
    val code = """
      ...
      cmd"baz -i $store2 -o $store3"(in = Seq(store2), out = Seq(store3))
      """
    ...
    val code = """
      ...
      andThen {
        ...
        for(i <- 1 to N) {
          ...
          cmd"nuh -i $store3 -o $nuhOutput"(in = Seq(store3), out = Seq(nuhOutput))
        }
      }
      """
  ...
  private def toPath(g: LoamGraph)(store: Store): Path = store.path

  private def assertStoreExists(g: LoamGraph)(expectedPath: String): Unit = {
    assert(g.stores.exists(s => toPath(g)(s) == Paths.get(expectedPath)))
  }

  private def findStore(g: LoamGraph)(expectedPath: String): Store = {
    g.stores.find(s => toPath(g)(s) == Paths.get(expectedPath)).get.asInstanceOf[Store]
  }
