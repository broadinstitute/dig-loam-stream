...
final class LoamToolBoxTest extends FunSuite {
  ...
  test("Simple toy pipeline using cp, some tools named; ensure tool names are propagated to jobs.") {
    ...
    assert(jobInToTmp1.name.startsWith(LoamCmdTool.autogeneratedNamePrefix))
    assert(jobTmp1ToTmp2.name.startsWith(LoamCmdTool.autogeneratedNamePrefix))
    assert(jobTmp2To1.name.startsWith(LoamCmdTool.autogeneratedNamePrefix))
