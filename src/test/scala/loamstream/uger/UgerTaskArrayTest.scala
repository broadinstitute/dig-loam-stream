...
object UgerTaskArrayTest {
  ...
  val outputDir = path("/path/to/logs")
...
final class UgerTaskArrayTest extends FunSuite {
  ...
  test("ugerStdOutPathTemplate") {
    doPathTemplateTest(
      ...
      s":/foo/bar/baz/blarg-blahblah.${JobTemplate.PARAMETRIC_INDEX}.stdout")
  }
  
  test("ugerStdErrPathTemplate") {
    doPathTemplateTest(
      UgerTaskArray.ugerStdErrPathTemplate,
      s":/foo/bar/baz/blarg-blahblah.${JobTemplate.PARAMETRIC_INDEX}.stderr")
  }
